...
function App() {
  ...
  try {
    ...
    // Fetch detailed data from TMDB for each recommended movie title
    const detailedMovies = await Promise.all(
      movieTitles.map(async (title) => {
        const searchResponse = await fetch(
          `${API_BASE_URL}/search/movie?query=${encodeURIComponent(title.split(' (')[0])}`, // Extract just the title
          API_OPTIONS
        );
        if (!searchResponse.ok) throw new Error(`Failed to fetch data for ${title}`);
        
        const searchData = await searchResponse.json();
        return searchData.results[0] || { title }; // Fallback to title if TMDB fails
      })
    );
